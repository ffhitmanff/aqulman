...
INGAT JANGAN JUAL SCRIPT HITMAN KEPADA ORANG LAIN!
...
module.exports = tobz = async (tobz, message) => {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh HITMAN', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh HITMAN!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev HITMAN*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev HITMAN*`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev HITMAN*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev HITMAN*`, id)
            }
            break
...
• *Link:* https://twitter.com/${user.screen_name}`
        ...
        case prefix+'igstalk':
            ...
            console.log(istalk)
            try {
            const istalk2 = await axios.get('https://api.vhtear.com/igprofile?query=${ige}&apikey=Jsieu8287362jshre82')
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN HITMAN
        ...
        case prefix+'limit':
            ...
            break
        case prefix+'gift': // Hanya Admin & Owner HITMAN yang bisa gift Limit
...
• Limit: ${limitCount-updated.limit}`
	...
	//list menu
        ...
        case prefix+'HITMANgroup':
            tobz.reply(from, `Link Group HITMAN : https://chat.whatsapp.com/By906EiJBGBCZGURDadOat\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup HITMAN, Link Ada Di Bio atau Bisa Mengetik #HITMANgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
